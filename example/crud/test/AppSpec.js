describe('Rally.example.CRUDTesting', function() {
    ...
    beforeEach(function() {
      ...
      update = Rally.test.Mock.ajax.whenUpdating('defect', defect.ObjectID).respondWith(Ext.apply({}, defect, {
        State: 'Fixed'
      }));
    });

    pit('it should create a defect', function() {
      var app = Rally.launchApp('Rally.example.CRUDTesting', {});  //todo: better way to launch app?
    ...
    pit('it should read the defect', function() {
      var app = Rally.launchApp('Rally.example.CRUDTesting', {});
